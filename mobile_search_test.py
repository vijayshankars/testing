#!/usr/bin/env python3
"""
Mobile Number Search Functionality Testing for Admin Dashboard
Tests the specific mobile search feature as requested in the review
"""
...
def test_mobile_number_search_functionality():
    ...
    admin_token = None
    admin_phone = "+91 8888888888"  # Use different number to avoid conflicts
    ...
    try:
        verify_data = {
            ...
            "otp_code": demo_otp,
            "user_type": "admin",
            "name": "Test Admin User Mobile Search"
        }
                ...
                if send_data.get("success"):
                    ...
                    if verify_response.status_code == 200:
                        ...
                        if verify_data.get("success"):
                            ...
                            if "already" in verify_data.get("message", "").lower() or not verify_data.get("is_new_user", True):
                                created_users.append(user_data)
                                result.log_success(f"Step 2{chr(97+i)} - User {user_data['phone']} already exists (OK)")
                            else:
    ...
    # Test 3c: Search for "888" (should return admin user)
    try:
        params = {"mobile_search": "888"}
        response = make_request("GET", "/admin/users", headers=admin_headers, params=params)
        if response.status_code == 200:
            data = response.json()
            if "users" in data and isinstance(data["users"], list):
                # Should return admin user with 8888888888
                found_phones = [user.get("phone", "") for user in data["users"]]
                
                if any("8888888888" in phone for phone in found_phones):
                    result.log_success("Step 3c - Mobile search '888' returns admin user")
                else:
                    result.log_failure("Step 3c", f"Expected admin user for '888', found phones: {found_phones}")
            else:
            ...
            if "users" in data and isinstance(data["users"], list):
                ...
                if len(rider_matches) >= 1:
                    ...
                    if "9876543210" in rider_phone:
                        ...
                        result.log_success("Step 4a - Combined filter returns rider (phone format may vary)")
                else:
            ...
            if "users" in data and isinstance(data["users"], list):
                ...
                if len(driver_matches) >= 1:
                    ...
                    if "9876543211" in driver_phone:
                        ...
                        result.log_success("Step 4b - Combined filter returns driver (phone format may vary)")
                else:
